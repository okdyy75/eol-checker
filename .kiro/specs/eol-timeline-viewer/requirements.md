...
**ユーザーストーリー:** ユーザーとして、入力したデータがブラウザを閉じても保持されることを期待する。そうすることで、毎回入力し直す必要がなくなる。

#### 受入基準

1. THE System SHALL 入力されたデータをブラウザのローカルストレージに保存する
2. WHEN ユーザーがアプリケーションを再度開いた場合、THE System SHALL ローカルストレージからデータを読み込む
3. WHEN URL_Stateが存在する場合、THE System SHALL URL_Stateをローカルストレージよりも優先する
4. THE System SHALL ユーザーがデータをクリアできる機能を提供する
