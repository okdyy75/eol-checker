...
- 現在使用中バージョンのマーカー表示（★）
- バーの期間がリリース日と終了日定義（EOL日、またはEOL未設定時のfallback）に正確に一致
- **URL共有機能**
...
- **ライフサイクル表示**
- 各バージョンのバーをリリース日から終了日までの期間として表示
- 終了日は原則EOL日。EOL未設定時はsupport日、supportも未設定時は現在日+5年を暫定終了日とする
- バーの開始日と終了日が実際のリリース日と終了日定義に正確に一致
  - ライフサイクルステージごとにセグメント分割
  - ステージごとの色分け表示（current: 緑、active: 青、maintenance: グレー、eol: 赤）
- **視覚的要素**
  - 現在使用中バージョンのマーカー表示（★）
  - 現在日付を示す垂直線の表示
  - ツールチップ表示（バージョン番号、リリース日、EOL日）
  - ライフサイクルステージの凡例表示
- **レスポンシブ対応**
  - 画面サイズに応じた最適化
...
*任意の*バージョンに対して、ガントチャートのバーの開始日はリリース日と正確に一致し、終了日は終了日定義（EOL日、またはEOL未設定時のfallback）と正確に一致する
**検証: 要件 3.3, 3.10**
...
- サポート終了日なし → `active`（リリース→EOL、EOL未設定時は暫定終了日）
